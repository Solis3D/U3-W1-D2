...
const MyLibrary = function () {
  ...
  const handleSelectBook = function (asin) {
  ...
  const searchedBooks = books.filter((libro) => libro.title.toLowerCase().includes(search.searchBook.toLowerCase()));
  return (
    <>
      <Form>
      ...
      <Row className=" my-5 px-0 px-md-3">
        <Col className="d-block m-auto" xs={12} lg={8}>
          <Row className="g-2 gap-3 justify-content-center">
            {searchedBooks.map((libro) => {
              return <SingleBook key={libro.asin} book={libro} selectedAsin={selectedAsin} onSelect={handleSelectBook} />;
            })}
          </Row>
        </Col>
        <Col
        ...
        >
          ...
          <CommentArea bookAsin={selectedAsin} />
        </Col>
      </Row>
    </>
  );
